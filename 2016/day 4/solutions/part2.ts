...
export function part2(input: string): string {
  ...
  const realRooms = validLines.map((line) => {
    const letters = line.string.split("");
    const decryptedLetters = letters.map((letter) => {
      ...
      const newCharCode =
        ((charCode - aCharCode + line.sectorId) % 26) + aCharCode;
      return String.fromCharCode(newCharCode);
    });
    return decryptedLetters.join("");
  });
  const northPoleRoom = realRooms.find((room) => room.includes("northpole"));
  if (!northPoleRoom) {
    throw new Error("No room found");
  }
  const northPoleRoomIndex = realRooms.indexOf(northPoleRoom);
  return sectorIds[northPoleRoomIndex].toString();
}
